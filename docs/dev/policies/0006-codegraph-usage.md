...
- Whenever codegraph is needed for the task and the intended repository or worktree root is verified, initialization is routine derived-state maintenance: run the documented initialization workflow and verify the resulting index status. This permission applies whether the index is missing, the worktree is new, or the repository has not been initialized before; do not require separate approval solely to initialize codegraph.
...
- Before initialization, confirm the target root and repo-local exclusions. Stop and ask only when the target or allowed input scope is ambiguous, repo policy explicitly reserves indexing for an operator, or initialization would create unexpected tracked-file changes.
...
Use available CodeGraph or codebase-memory-mcp structural tools before symbol,
call-flow, or impact exploration. Native search is appropriate for configs,
policy text, literal strings, and graph gaps. Respect the operator's existing
rule to ask before initializing a missing CodeGraph index; it overrides the
shared automatic-initialization suggestion. Query the intended worktree, not
the maintenance index for v1. Use SysRAG for semantic filesystem recall only
within already authorized roots; never register additional roots implicitly.
Keep exact commands, MCP tool names, sibling checkout paths, service repair, and project-specific index exclusions repo-local. The reusable contract is initialize whenever codegraph is necessary and the target is verified, explicitly sync when automatic refresh is absent, and verify current status.
